...
// frontend registry can fetch /modules/yz-body.manifest.json and the Python
...
// Layout: satellites/yz-body/ui/scripts/ -> climb 4 levels to project root
const projectRoot = resolve(here, '..', '..', '..', '..')
const satelliteRoot = resolve(here, '..', '..')

const iifeSrc = resolve(here, '..', 'dist-lib', 'yz-body.iife.js')
const manifestSrc = resolve(satelliteRoot, 'manifest.json')

const iifeTargets = [
  resolve(projectRoot, 'frontend', 'public', 'modules', 'yz-body.iife.js'),
  resolve(projectRoot, 'backend', 'jarvyz', 'web', 'static', 'modules', 'yz-body.iife.js'),
]
const manifestTargets = [
  resolve(projectRoot, 'frontend', 'public', 'modules', 'yz-body.manifest.json'),
  resolve(projectRoot, 'backend', 'jarvyz', 'web', 'static', 'modules', 'yz-body.manifest.json'),
]
...
if (existsSync(manifestSrc)) {
  ...
  if (missing.length) {
    console.error(
      ...
      `Check satellites/yz-body/ui/src/index.ts.`,
    )
    process.exit(1)
  }
